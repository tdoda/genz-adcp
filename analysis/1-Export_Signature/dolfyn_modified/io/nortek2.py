...
def read_signature(
    ...
    **kwargs,
    ...
    if "orientmat" not in ds:
        ...
        # )
        # Modification to deal with only echo variable names (T. Doda, 25.07.2025)
        if "heading" in ds:
            ds["orientmat"] = _euler2orient(
                ds["time"], ds["heading"], ds["pitch"], ds["roll"]
            )
            ...
            ds["orientmat"] = _euler2orient(
                ds["time"], ds["heading_echo"], ds["pitch_echo"], ds["roll_echo"]
            )    
...
def _reorg(dat):
    ...
    if 21 not in dat:
        ...
        # Modification to deal with other variable names (T. Doda, 25.07.2025):
        if "coord_sys_axes_avg" in cfg.keys():
            cy = cfg["coord_sys_axes_avg"]
        elif "coord_sys_axes_echo" in cfg.keys():
            cy = cfg["coord_sys_axes_echo"]
        else:
            raise Exception
    else:
        cfg["rotate_vars"] = [
...
def _clean_dp_skips(data):
    ...
    for id in data:
        if id == "filehead_config" or type(id)==int: # Modified by T.Doda: remove integer indices that do not correpond to datasets
            continue
...
def _reduce(data):
    ...
    # Addition to deal with only echo data (T. Doda, 25.07.2025):
    if ("vel" not in dv) and ("vel_avg" not in dv): # Only Echo data
        da["fs"] = da["filehead_config"]["BURST"]["SR"]
        tmat = da["filehead_config"]["XFBURST"]
    if "vel_b5" in dv:
    ...
    # Addition to deal with only echo data: do not compute beam2inst_orientation (T. Doda, 25.07.2025):
    if ("vel" in dv) or ("vel_avg" in dv): 
